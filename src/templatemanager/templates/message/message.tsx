...
export function Message(props: any) {
    ...
    if (msgData?.message[0]?.component?.payload?.template_type === 'live_agent') {
        ...
        if (!msgData.fromHistory && msgData.author) {
            ...
            <Fragment>
                {
                    msgData.message.map((msgItem: any) => (
                        ...
                        <div class={`message-bubble i${msgData.messageId || msgItem.clientMessageId}`} data-time-stamp={msgData.createdOnTimemillis} id={msgData.messageId || msgItem.clientMessageId}>
                           {/* {msgItem.component?.payload?.text?.includes("Brand: MAKO") && (
  <div className="boat-card-list">
    {msgItem.component.payload.text
      .split(/(?=\d+\.\s)/g) // split when line starts with "1. ", "2. ", etc.
      .slice(1) // skip the first intro line
      .map((chunk: string, index: number) => {
        const titleMatch = chunk.match(/^(\d+)\.\s(.+?)\s\((https?:\/\/[^\s)]+)\)/);
        const priceMatch = chunk.match(/Price:\s*\$([\d,]+)/);
        const brandMatch = chunk.match(/Brand:\s*(.+)/);
        const descriptionMatch = chunk.match(/Description:\s*(.+)/);
        
        if (!titleMatch || !priceMatch || !brandMatch || !descriptionMatch) return null;
        ...
        const imageUrl = imageMatch?.[1] || "https://via.placeholder.com/150";

        return (
          <div key={index} className="boat-card">
            <img src={imageUrl} alt={title} className="boat-image" />
  ...
  msgItem.component?.payload?.text?.includes("Brand: MAKO") ? (
    <div className="boat-card-list">
      {msgItem.component.payload.text
        ...
        .map((chunk: string, index: number) => {
          ...
          const brandMatch = chunk.match(/Brand:\s*(.+)/);
          const descriptionMatch = chunk.match(/Description:\s*(.+)/);
          const skuMatch = chunk.match(/SKU:\s*(.+)/);
          const imageMatch = chunk.match(/Image:\s*Image\)\)\/(.+?)\.json\)/i);
          ...
          const imageUrl = imageMatch
            ? `https://example.com/images/${imageMatch[1]}.jpg` // <-- Replace with real path
    ...
    <div
      ...
      style={{ color: msgItem.component.payload.color }}
      dangerouslySetInnerHTML={{
        __html: helpers.convertMDtoHTML(msgItem.component.payload.text, "bot", msgItem),
      }}
    />
  )
}
                                            ...
                                            {brandingInfo.body.icon.show && !msgData.fromAgent && <div className="bot-img">
                                                <figure>
                                                    {/* <img src={msgData && msgData.icon ? msgData.icon : iconHelper.getIcon('retail')} alt='avatr img' /> */}
                                                    <img src={msgData && msgData.icon ? iconHelper.getIcon('retail') : iconHelper.getIcon('retail')} alt='avatr img' />
                                                </figure>
                                            </div>}
                                            {brandingInfo.body.icon.show && msgData.fromAgent && <div className="bot-img">
                                                <figure>
                                                    <img src={msgData && msgData.icon ? iconHelper.getIcon('retail') : brandingInfo.body.agent_message.icon.icon_url} alt='avatr img' />
